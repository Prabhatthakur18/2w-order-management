...
import { verifyGstin, createSubDealer } from "../../../actions";
...
/**
 * GSTIN drives auto-fill via a Verify button, same as the dealer form —
 * sub-dealers are firms in their own right. Every field stays editable
 * afterwards; submitting always creates a PENDING sub-dealer.
 */
export function NewSubDealerForm({ dealerId }: { dealerId: string }) {
  ...
  function handleSubmit() {
    ...
    setFieldErrors({});
    startSubmit(async () => {
      const result = await createSubDealer({ dealerId, ...form });
      if (result.ok) {
        router.push("/asm/dealers");
      } else {
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <p className="text-center text-xs text-muted-foreground">
        Admin will review this before it can be used on orders.
      </p>
    </div>
  );
}
