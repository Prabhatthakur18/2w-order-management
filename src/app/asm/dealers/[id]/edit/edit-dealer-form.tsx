...
import { verifyGstin, updateDealer } from "../../actions";
...
/**
 * Editing an already-APPROVED/REJECTED dealer resets it to PENDING for
 * Admin re-review — same GST-verify + field layout as the create form,
 * pre-populated and calling updateDealer instead of createDealer.
 */
export function EditDealerForm({
  ...
  function handleSubmit() {
    ...
    setFieldErrors({});
    startSubmit(async () => {
      const result = await updateDealer(dealerId, form);
      if (result.ok) {
        router.push(`/asm/dealers/${dealerId}`);
      } else {
  ...
  return (
    <div className="space-y-4 pb-8">
      ...
      <Card>
        <h4 className="mb-3 text-sm font-bold">Dealer details</h4>
      ...
      <p className="text-center text-xs text-muted-foreground">
        If this dealer was already approved, saving changes sends it back to
        Admin for re-review.
      </p>
    </div>
  );
